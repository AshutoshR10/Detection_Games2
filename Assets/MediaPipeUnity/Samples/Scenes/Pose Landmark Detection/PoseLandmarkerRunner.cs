...
namespace Mediapipe.Unity.Sample.PoseLandmarkDetection
{
  public class PoseLandmarkerRunner : VisionTaskApiRunner<PoseLandmarker>
  {
    ...
    // Cache PoseResultAdapter reference to avoid FindObjectOfType on callback thread
    private PoseInteraction.Core.PoseResultAdapter _poseResultAdapter;

    public readonly PoseLandmarkDetectionConfig config = new PoseLandmarkDetectionConfig();

    protected void Awake()
    {
      // Cache the adapter reference on main thread (Awake is called before Start)
      _poseResultAdapter = FindFirstObjectByType<PoseInteraction.Core.PoseResultAdapter>();
    }
    ...
    private void OnPoseLandmarkDetectionOutput(PoseLandmarkerResult result, Image image, long timestamp)
    {
      ...
      // Use cached adapter reference (safe for callback thread)
      _poseResultAdapter?.ForwardResult(result, image.Width(), image.Height());
    }
